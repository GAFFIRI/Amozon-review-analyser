...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            eta = 1/math.sqrt(update)
            data = pegasos_single_step_update(feature_matrix[i],labels[i],eta,L,current_theta,current_theta_0)
            update = update +1 
            current_theta = data[0]
            current_theta_0 = data[1]
    return (current_theta,current_theta_0)
